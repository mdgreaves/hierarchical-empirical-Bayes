...
for i = length(P)
    ...
    assert(all(1./p(~eye(P{1}.n)) == 1/p(2)), ['To follow procedures ',...
        'in the associated study, the precision of subject-level ',...
        'priors over all state transitions, when i ~= j, should be ',...
        'equal.']);
    assert(all(round(1./p(~(~eye(P{1}.n)))) == 64), ['To follow ',...
        'procedures in the associated study, the precision of ',...
        'subject-level priors over state transitions, when i == j, ',...
        'should equal 64.']);
end
...
if ~exist('HEB', 'var')
    ...
    % Generate alpha (intercept) values ranging from -p to p
    params.alphas = linspace(-max(p(~eye(P{1}.n))),...
        max(p(~eye(P{1}.n))), num_points);

    % Initialize beta (slope) matrix
    params.betas = zeros(num_points, num_points);

    % For each alpha, generate beta ensuring that alpha + beta <= p
    for i = 1:num_points
        params.betas(i, :) = linspace(0, max(p(~eye(P{1}.n)))...
            - params.alphas(i), num_points);
    end
